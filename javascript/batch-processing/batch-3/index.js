...
async function jobRunner(getJobFn, processingFunction, index) {
  // console.log('jobRunner -> ', index)
  let processing = 1;
  while(true) {
    const job = getJobFn.next()
    if(job.done) {
      logEndTime(startTime)
      break
    }
    if(job.value) {
      // console.log('jobRunner -> %s processing = %s', index, processing)
      const result = await processAJob(job.value, processingFunction)
      // console.log('jobRunner -> %s result = ', index, result)
    }
  }
}
...
async function run(batchJobs, batchSize, processingFunction) {
  ...
  const jobGenerator = getJob(batchJobs)
  startTime = performance.now();

  for (let jobRunnerIndex = 0; jobRunnerIndex < batchSize; jobRunnerIndex++) {
    jobRunner(jobGenerator, processingFunction, jobRunnerIndex) // Execution time: 6009.6449999999995 milliseconds
  }
  ...
  for (let batchIndex = 0; batchIndex < numOfBatches; batchIndex++) {
    ...
    const batchProcessingResults = await processABatchSynchronous(batch, processingFunction);
    // const batchProcessingResults = await processABatchAsynchronous(batch, processingFunction);
...
run(ordersData, 5, updateOrderPrice)
